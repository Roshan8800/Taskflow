      ...
      {['all', 'pending', 'completed'].map(status => (
        <TouchableOpacity
          ...
          style={{
            ...
            paddingVertical: 6,
            backgroundColor: statusFilter === status ? project.color : '#f0f0f0',
            borderRadius: 16,
          }}
        >
      ...
      {['all', 'urgent', 'high', 'medium', 'low'].map(priority => (
        <TouchableOpacity
          ...
          style={{
            paddingHorizontal: 12,
            paddingVertical: 6,
            backgroundColor: priorityFilter === priority ? project.color : '#f0f0f0',
            borderRadius: 16,
          }}
        >
    ...
    <ScrollView style={{ flex: 1, backgroundColor: '#f5f5f5' }}>
      ...
      <View style={{ backgroundColor: 'white', padding: 20, marginBottom: 16 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 12 }}>
          <Text style={{ fontSize: 32, marginRight: 12 }}>{project.icon}</Text>
          <View style={{ flex: 1 }}>
            <Text style={{ fontSize: 24, fontWeight: 'bold' }}>{project.name}</Text>
            {project.description && (
              <Text style={{ color: '#666', marginTop: 4 }}>{project.description}</Text>
            )}
          </View>
        </View>
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
          <View style={{ alignItems: 'center' }}>
            <Text style={{ fontSize: 24, fontWeight: 'bold', color: project.color }}>{stats.total}</Text>
