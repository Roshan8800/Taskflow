...
export const ProjectCard: React.FC<ProjectCardProps> = ({ projectId, onPress, onEdit, onDelete }) => {
  const { getProjectStats } = useProjectManager();
  const stats = getProjectStats(projectId._id);
  ...
  return (
    <TouchableOpacity onPress={onPress} style={{ margin: 8 }}>
      <View style={{
        ...
        elevation: 3,
        borderLeftWidth: 4,
        borderLeftColor: projectId.color,
      }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
          <Text style={{ fontSize: 24, marginRight: 8 }}>{projectId.icon}</Text>
          <Text style={{ fontSize: 18, fontWeight: 'bold', flex: 1 }}>{projectId.name}</Text>
          {projectId.archived && (
        ...
        {projectId.description && (
          <Text style={{ color: '#666', marginBottom: 12, fontSize: 14 }}>{projectId.description}</Text>
        )}
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <View style={{ flexDirection: 'row', gap: 8 }}>
            <TouchableOpacity onPress={onEdit} style={{ padding: 8 }}>
              <Text style={{ color: projectId.color, fontWeight: '600' }}>Edit</Text>
            </TouchableOpacity>
